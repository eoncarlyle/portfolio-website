...
let private mergeWhitespaceTokens (tokens: ThemedToken[][]) : ThemedToken[][] =
    tokens |> Array.map (fun line ->
        ...
        for idx = 0 to line.Length - 1 do
            ...
            if couldMerge
               && reWhitespaceOnly.IsMatch(token.Content)
               && idx + 1 < line.Length then
                if firstOff.IsNone then firstOff <- ValueSome token.Offset
                carry <- carry + token.Content
            else
                if carry <> "" then
                    if couldMerge then
                        newLine.Add { token with
                                        Offset  = firstOff.Value
                                        Content = carry + token.Content }
                    else
                        newLine.Add { Content   = carry
                                      Offset    = firstOff.Value
                                      Color     = None
                                      BgColor   = None
                                      FontStyle = 0 }
...
let tokensToHtml
    ...
    for lineIdx = 0 to tokens.Length - 1 do
        ...
        for token in tokens[lineIdx] do
            ...
            if style <> "" then
                sb.Append($"""<span style="{style}">{htmlEscape token.Content}</span>""") |> ignore
            else
