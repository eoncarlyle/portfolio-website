...
```
This took an embarrassing amount of time to get working, so much so that I would have probably done something else that weekend (computing or otherwise) if I had known what a hassle it would be. Some of the issues that I ran into were

- Before the `/etc/hosts` was updated on the VPS, clients on that server would not work because the `PREROUTING` chain rules aren't applied to packets coming from the VPS. So hostname resolution fell through to public DNS, and it tried to connect to itself on ports 10000 and 10001 where nothing was listening. This didn't apply to clients outside the VPS.
- When `161.35.106.161` was used in place of `broker-X.santa-cruz-kafka-1.iainschmitt.com` in `KAFKA_ADVERTISED_LISTENERS`, clients on the VPS didn't work because this bypassed the `/etc/hosts` fix and reintroduced another hairpin.
- When I misspelled `security.protocol` as `ecurity.protocol` in the `application.properties` file used by `kafka-topics.sh`, the program failed with a `OutOfMemoryError` rather than with an exception that actually pointed out what was going on.[^claude-commentary]
